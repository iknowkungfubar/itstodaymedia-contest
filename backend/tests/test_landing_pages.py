...
class TestLandingPages:
    ...
    def test_create_landing_page(self, client: TestClient) -> None:
        ...
        response = client.post("/api/landing-pages", json={
    ...
    def test_performance_summary(self, client: TestClient, db_session: Session) -> None:
        ...
        pages = [
            LandingPageModel(campaign_id=campaign_id, platform="meta",
                            url="https://example.com/a", visits=1000,
                            conversions=50, revenue=500.0, cost=100.0),
            LandingPageModel(campaign_id=campaign_id, platform="meta",
                            url="https://example.com/b", visits=500,
                            conversions=40, revenue=400.0, cost=80.0),
        ]
        for p in pages:
            db_session.add(p)
        db_session.commit()
